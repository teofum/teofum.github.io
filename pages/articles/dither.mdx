...
The graph takes a one-dimensional “slice” of an image and placing brightness along the y axis. The *`white line`* indicates the threshold, while the points connected by the `orange line` represent points in the image. The color of each point is its value after thresholding. The *target* box shows the “real” color, while the “sample average” box shows the average of all sampled points *after* thresholding:
