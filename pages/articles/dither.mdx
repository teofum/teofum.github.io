...
Modern displays and image formats can display some 16.7 *million* colors. That's a lot. That hasn't always been the case, though. Back in the early days of computers, many devices supported as few as 16 or even just *four* colors. Even in the late 1990s to early 2000s many people still had 256-color displays—hence the 216-color “web safe” palette. Many image formats—notably the widely used GIF—were limited to 256 colors as well.

In these low-color scenarios, dithering helps reduce color banding and effectively multiplies the number of colors we can display. Generally speaking, dithered images just look much better (click each image to view in detail):

<ImageGrid pixelated images={[
  {
    src: '/content/articles/dither/boat-original.webp',
    alt: 'Original image',
    caption: '24-bit “true color”'
  },
  {
    src: '/content/articles/dither/boat-web.webp',
    alt: 'Web safe image',
    caption: 'Web safe palette (216 colors)'
  },
  {
    src: '/content/articles/dither/boat-web-dither.webp',
    alt: 'Dithered image',
...
<ImageGrid noScroll images={[
  {
    src: '/content/articles/dither/tram-original.webp',
    alt: 'Original image'
  }
]} />
...
<ImageGrid pixelated noScroll images={[
  ...
  {
    src: '/content/articles/dither/tram-ed-simple.webp',
...
<ImageGrid minSize='30ch' pixelated images={[
  {
    src: '/content/articles/dither/tram-ed-fs.webp',
    alt: 'Floyd-Steinberg',
    caption: 'Floyd-Steinberg'
  },
  {
    src: '/content/articles/dither/tram-ed-jjn.webp',
    alt: 'Minimum Average Error',
    caption: 'Minimum Average Error'
  },
  {
    src: '/content/articles/dither/tram-ed-stucki.webp',
    alt: 'Stucki',
    caption: 'Stucki'
  },
  {
    src: '/content/articles/dither/tram-ed-sierra.webp',
    alt: 'Sierra',
    caption: 'Sierra'
  }
]} />
...
<ImageGrid pixelated noScroll images={[
  {
    src: '/content/articles/dither/flower.webp',
...
<ImageGrid pixelated noScroll images={[
  {
    src: '/content/articles/dither/tram-motion-original.webp',
    alt: 'Error dithering animation',
    caption: 'Original animation, with a single moving pixel'
  },
  {
    src: '/content/articles/dither/tram-motion-ed.webp',
    alt: 'Error dithering animation',
...
for (x, y) in input
  value = input[x, y]
  noise = k * (random(0, 1) - 0.5)
  output[x, y] = if (value + noise < t) 0; else 1
```
...
We also lose the parameters `t` and `k`, which control the noise profile in the previous example. Here, the threshold map provides all the information we need.
...
Generating blue noise is outside the scope of this article, but you can learn more [here](http://momentsingraphics.de/BlueNoise.html). For our purposes, we can just use pre-generated blue noise textures as a threshold map (the [linked site](http://momentsingraphics.de/BlueNoise.html) has plenty available to download). You can try out ordered dithering with a blue noise threshold map below:
