...
The graph takes a one-dimensional “slice” of a flat gray color shown in the “target” box, placing brightness along the y axis. The *`white line`* indicates the threshold, while the points connected by the `red line` represent points in the image. The color of each point is its value after thresholding. The “sample average” box shows the average of all sampled points *after* thresholding.
