...
import styles from './DemoRandomGraph.module.scss';

// Precalculated random array
const noise = [
  0.3355752696192672,
  -0.0885945672868057,
  -0.018366641617918233,
  -0.2807422461531063,
  -0.3201837834294239,
  0.12823271209396525,
  0.1962856153741076,
  ...
  -0.427941766648418,
  -0.09578367337916394,
  -0.006558660890175427,
  0.2740267067910015,
  0.2542480373602639,
  -0.15523101377267867,
  -0.024223832665937306,
  0.000776939924034159,
  -0.23021991930076813,
  -0.2977072935379641,
  0.031175256669283224,
  -0.4548917373774737,
  0.007528872261648889,
  0.429868005089739,
  0.14803793582891622,
  0.40553381473901784,
  0.42011071990376325,
  ...
  0.3300856954260788,
  0.3799861180061199,
  0.041487691887627465,
  -0.48604789958013384,
  -0.008770841851630906,
  0.320445655819085,
  -0.011126017349203932,
  0.15204090630685962,
  -0.47596975966316823,
  0.31299055069819404,
  -0.4706185477860302,
  ...
  -0.43871668425424515,
  0.42145944851142547,
  0.012765703465875244,
  -0.040889897125666264,
  -0.07953310161756055,
  -0.09619504730856199,
  0.44313034290783726,
  0.44724598054521436,
  ...
  0.2519874983604051,
  0.11353676875789542,
  0.015049153406938909,
  0.35482980000496256,
  0.1101280651688773,
  0.36434851247529065,
  0.052813477648619656,
  0.21930838390325358,
  -0.38927827920711067,
  -0.4211955053925476,
  0.25197695784196406,
...
const DemoThreshold = () => {
  ...
  useEffect(() => {
    ...
    const size = Math.max(
      2 * scaling,
      Math.min(6 * scaling, w / points.length / 2 * 0.75)
    );
  ...
  const average = points
    .map(point => brightness - point * k)
    .filter(point => point > 0.5)
    ...
    <div className={demoStyles.container}>
      <div className={cn(
        ...
        <div className={styles.colorPreview}>
          <div style={{ backgroundColor: `hsl(0, 0%, ${brightness * 100}%)` }} />
          <span>Target</span>
          <div style={{ backgroundColor: `hsl(0, 0%, ${average * 100}%)` }} />
          <span>Sample average</span>
        </div>

        <canvas className={cn(demoStyles.canvas, styles.canvas)}
          ref={el => canvas = el} />
      </div>
      ...
      <div className={demoStyles.controls}>
        <label>
          Brightness
          <input type='range' autoComplete='off' value={brightness} min={0} max={1} step={0.005}
            onChange={e => setBrightness(parseFloat(e.target.value))} />
        </label>

        <label>
          Samples
          <input type='range' autoComplete='off' value={n} min={15} max={150} step={1}
